...
If the user wants to add or manage briefing topics, use the sub-agent management tool. Valid topic types are: market_tracker, network_activity, web_search, github_activity, rss_feed, and custom. Use web_search for topics that need current information from the internet (news, weather, events, etc.). For github_activity, the config should include a "repos" array of "owner/repo" strings (e.g. ["vercel/next.js"]). For rss_feed, the config should include a "urls" array of feed URLs and an optional "max_items" number (e.g. { "urls": ["https://feeds.example.com/rss"], "max_items": 10 }).
